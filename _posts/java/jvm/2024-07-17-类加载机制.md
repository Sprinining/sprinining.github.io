---
title: 类加载机制
date: 2024-07-17 08:33:23 +0800
categories: [java, jvm]
tags: [Java, JVM, Class Loader]
description: 
---
## 类的生命周期
...
![java_jvm_classload_2](/assets/media/pictures/java/类加载机制.assets/java_jvm_classload_2.png)
...
![java_jvm_classload_1](/assets/media/pictures/java/类加载机制.assets/java_jvm_classload_1.png)
...
![java_jvm_classload_3](/assets/media/pictures/java/类加载机制.assets/java_jvm_classload_3.png)

#### Java虚拟机的角度

只存在两种不同的类加载器: 启动类加载器: 它使用C++实现(这里仅限于`Hotspot`，也就是JDK1.5之后默认的虚拟机，有很多其他的虚拟机是用Java语言实现的)，是虚拟机自身的一部分；所有其他的类加载器: 这些类加载器都由Java语言实现，独立于虚拟机之外，并且全部继承自抽象类`java.lang.ClassLoader`，这些类加载器需要由启动类加载器加载到内存中之后才能去加载其他的类。
...
public static void main(String[] args) {
    ...
    // null
    // 并没有获取到ExtClassLoader的父Loader，原因是BootstrapLoader(引导类加载器)是用C语言实现的，找不到一个确定的返回父Loader的方式，于是就返回null。
...
public class Test {
    public static void main(String[] args) throws ClassNotFoundException {
        ...
        // loader.loadClass("test.GCTest.Test2");

        // 使用Class.forName()来加载类，默认会执行初始化块
        // Class.forName("test.GCTest.Test2");

        // 使用Class.forName()来加载类，并指定ClassLoader，初始化时不执行静态块
        Class.forName("test.GCTest.Test2", false, loader);
    }
}
...
class MyClassLoader extends ClassLoader {
    ...
    public static void main(String[] args) {
        ...
        Class<?> testClass;
        try {
            testClass = classLoader.loadClass("test.GCTest.Test");
...
1、这里传递的文件名需要是类的全限定性名称，即`test.GCTest.Test`格式的，因为 defineClass 方法是按这种格式进行处理的。

2、最好不要重写loadClass方法，因为这样容易破坏双亲委托模式。

3、这类Test 类本身可以被 AppClassLoader 类加载，因此我们不能把test.GCTest.Test 放在类路径下。否则，由于双亲委托机制的存在，会直接导致该类由 AppClassLoader 加载，而不会通过我们自定义类加载器来加载。
