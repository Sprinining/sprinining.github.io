---
title: Java IO模型
date: 2024-07-31 05:35:20 +0800
categories: [java, network programming]
tags: [Java, Network Programming, IO, BIO, NIO]
description: 
---
...
![img](/assets/media/pictures/java/JavaIO模型.assets/moxing-20230408110408.png)
...
![img](/assets/media/pictures/java/JavaIO模型.assets/moxing-20230408110433.png)

## 内核空间和用户空间

------

![img](/assets/media/pictures/java/JavaIO模型.assets/buffer-channel-selector-20230406182813.png)
...
- `select `是 Unix 系统中最早的 I/O 多路复用技术。它允许一个线程同时监视多个文件描述符（如套接字），并等待某个文件描述符上的 I/O 事件（如可读、可写或异常）。select 的主要问题是性能受限，特别是在处理大量文件描述符时。这是因为它使用一个==位掩码==来表示文件描述符集，每次调用都需要传递这个掩码，并在内核和用户空间之间进行复制。
...
![img](/assets/media/pictures/java/JavaIO模型.assets/moxing-20230408115158.png)
...
![img](/assets/media/pictures/java/JavaIO模型.assets/moxing-20230408115251.png)
...
![img](/assets/media/pictures/java/JavaIO模型.assets/moxing-20230408115328.png)
